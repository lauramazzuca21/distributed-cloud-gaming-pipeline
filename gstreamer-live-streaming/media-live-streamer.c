...
main (int argc, char *argv[])
{
  ...
  if (!data.pipeline || !data.source || !data.extract || !data.decode
      || !data.sink) {
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.source, data.extract,
      data.decode, data.sink, NULL);
  if (!gst_element_link_many (data.extract, data.decode, data.sink, NULL)) {
...
{
  GstPad *sink_pad = gst_element_get_static_pad (data->extract, "sink");
  ...
  /* If our extractor is already linked, we have nothing to do here */
